...
def visible_length(s):
    return len(re.sub(r'\x1b\[[0-9;]*[mG]', '', s))
...
class SrcOperand:
    ...
    def __init__( self, aOperand : str ):
        if aOperand[0:2] == 'RZ':
            ...
            if '+' in aOperand:
                self.mTp = self.Tp.REG_IMM
                split = aOperand.split('+')
                self.mVal = int( split[0][1:] )
                self.mImm = int( split[1], base=16 )
            else:
    ...
    def get_val( self, aWorkItem : Work_Item, aConstMem : bytearray, aType : int = 'I' ):
        match self.mTp:
            ...
            case self.Tp.ZERO:
                return 0
            case self.Tp.REG_IMM:
                val = aWorkItem.read_reg( self.mVal, aType=aType ) + self.mImm
                return val
            case _:
                assert 0
    ...
    def __init__( self, aInst : str, aLabel = "" ):
        ...
        match self.mOp:
            case (
                ...
                self.Op.LOP32I | self.Op.LEA | self.Op.FFMA
            ):
        ...
        match self.mOp:
            ...
            case (
                ...
                self.Op.STG | self.Op.SHL | self.Op.XMAD | self.Op.FMUL | self.Op.LOP32I |
                self.Op.LEA | self.Op.FFMA
            ):
        ...
        match self.mOp:
            case (
                ...
                self.Op.MOV32I | self.Op.SHL | self.Op.XMAD | self.Op.LOP32I | self.Op.LEA
            ):
                self.mDatapath = self.Datapath.INT
            case (
                self.Op.FFMA | self.Op.FMUL
            ):
...
def parse_instructions( aFilename:str, aKernelName:str ) -> typing.List[ Instruction ]:
    ...
    kernel = text.split( f".text.{aKernelName}:" )[-1].split("//---------------------")[0]
...
class Int_Datapath:
                ...
                match instruction.mOp:
                    ...
                    case Instruction.Op.XMAD:
                        ...
                        if "MRG" in instruction.mMods:
                            ...
                            val = product + op_c
                    case Instruction.Op.LOP32I:
                        op_a,op_b = [
                            op.get_val( item, warp.mConstMem ) for op in instruction.mSrc
                        ]
                        if "AND" in instruction.mMods:
                            val = op_a & op_b
                    # https://forums.developer.nvidia.com/t/how-to-understand-the-lea-assembly-behind-the-cuda-c/257679/2
                    case Instruction.Op.LEA:
                        op_a_lo,op_b,op_a_hi,imm_shift = [
                            op.get_val( item, warp.mConstMem ) for op in instruction.mSrc
                        ]
                        v_64 = op_a_lo | ( op_a_hi << 32 )
                        v_64 = ( v_64 << imm_shift )
                        if "HI" in instruction.mMods:
                            val = ( ( v_64 >> 32 ) & 0xffffffff ) + op_b
                    case _:
    ...
    def print( self, prefix='',postfix='',width=42 ):
        int_str = "|"
        ...
        ansi_len = len( int_str ) - visible_length( int_str )
        print(f"{prefix}{' INT  '+int_str:{width+ansi_len}}{postfix}")
    ...
    def warp_active( self, aWarp : Warp ) -> bool:
        for i in range( self.mIntLatency ):
            if self.mPipeline[i]:
                instr,warp = self.mPipeline[i]
                if warp == aWarp:
...
class Float_Datapath:
    ...
    def print( self, prefix='', postfix='', width=42 ):
        ...
        ansi_len = len( int_str ) - visible_length( int_str )
        print(f"{prefix}{' FLT  '+int_str:{width+ansi_len}}{postfix}")
...
class Memory_Datapath:
            ...
            for item in warp.mWorkItems:
                match instruction.mOp:
                    case ( Instruction.Op.STG ):
                        ...
                        #sprint(f"STG: Writing {val} to {addr}")
    ...
    def print( self, prefix='', postfix='', width=42 ):
        ...
        ansi_len = len( str ) - visible_length( str )
        print(f"{prefix}{' MEM  '+str:{width+ansi_len}}{postfix}")
...
class Scheduler:
    ...
    def clock( self ):
        ...
        for i in range( len( self.mActiveWarps ) ):
            warp = self.mActiveWarps[ ( self.mWarpIdx + i ) % len( self.mActiveWarps )  ]
            ...
            if instr.mDatapath:
                ...
                match instr.mDatapath:
                    case Instruction.Datapath.INT:
                        if not self.mIntDatapath.push( instr, warp ):
                            continue
                    case Instruction.Datapath.FLT:
                        if not self.mFloatDatapath.push( instr, warp ):
                            continue
                    case Instruction.Datapath.MEM:
                        if not self.mMemDatapath.push( instr, warp ):
                            continue
                    case _:
                        assert 0
            else:
                match instr.mOp:
                    ...
                    case Instruction.Op.EXIT:
                        if self.warp_active( warp ):
                            continue
                        self.mActiveWarps.remove( warp )
                    # Not implemented yet
                    case Instruction.Op.DEPBAR:
                        pass
                    case _:
                        assert 0

            warp.pop_instr()
            if not self.mMultiIssue:
                break
    ...
    def warp_active( self, aWarp : Warp ) -> bool:
        return (
            self.mIntDatapath.warp_active( aWarp=aWarp ) or
            self.mFloatDatapath.warp_active( aWarp=aWarp ) or
            self.mMemDatapath.warp_active( aWarp=aWarp )
        )

    def print( self, prefix='', postfix='', width=46 ):
        print(f"{prefix}{width*'-'}{postfix}")
        prefix = prefix + '| '
        postfix = ' |' + postfix
        width = width - 4

        print(f"{prefix}{'Processing Block':{width}}{postfix}")
        ...
        ansi_len = len( slot_str ) - visible_length( slot_str )

        print(f"{prefix}{' Active Warps:  '+slot_str:{width+ansi_len}}{postfix}")
        print(f"{prefix}{'                '+(self.mWarpInterleave*4+1)*'-':{width}}{postfix}")
        print(f"{prefix}{'':{width}}{postfix}")
        self.mIntDatapath.print( prefix=prefix, postfix=postfix, width=width )
        print(f"{prefix}{'':{width}}{postfix}")
        self.mFloatDatapath.print( prefix=prefix, postfix=postfix, width=width )
        print(f"{prefix}{'':{width}}{postfix}")
        self.mMemDatapath.print( prefix=prefix, postfix=postfix, width=width )
        print(f"{prefix}{'':{width}}{postfix}")
        prefix = prefix[:-2]
        postfix = postfix[2:]
        width = width + 4
...
class SM:
    ...
    def print( self, prefix='', postfix='', width=50 ):
        print(f"{prefix}{width*'-'}{postfix}")
        prefix = prefix + '| '
        postfix = ' |' + postfix
        width = width - 4
        print(f"{prefix}{' Streaming Multiprocessor':{width}}{postfix}")
        for scheduler in self.mSchedulers:
            scheduler.print( prefix=prefix, postfix=postfix )
        prefix = prefix[:-2]
        postfix = postfix[2:]
        width = width + 4
        print(f"{prefix}{width*'-'}{postfix}")
...
if __name__ == "__main__":
    ...
    sm.exec_kernel(
        ...
        aConstMem=cmem,
        aArgs=[
            np.arange( 384 ).astype( np.float32 ),
            np.arange( 384 ).astype( np.float32 )
        ]
    )
